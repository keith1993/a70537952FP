...
CREATE DEFINER=`root`@`localhost` PROCEDURE `fixed_expense_month` ()  NO SQL
begin
DECLARE U_ID int;  
DECLARE s int default 0;
DECLARE cursor_user CURSOR FOR select ID from user;  
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `fixed_income_day` ()  NO SQL
begin
DECLARE U_ID int;  
DECLARE s int default 0;
DECLARE cursor_user CURSOR FOR select ID from user;  
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `fixed_income_month` ()  NO SQL
begin
DECLARE U_ID int;  
DECLARE s int default 0;
DECLARE cursor_user CURSOR FOR select ID from user;  
...
-- 事件
--
CREATE DEFINER=`root`@`localhost` EVENT `event_fixedExpense_day` ON SCHEDULE EVERY 1 DAY STARTS '2016-11-06 00:00:00' ON COMPLETION PRESERVE ENABLE DO call fixed_expense_day()$$

CREATE DEFINER=`root`@`localhost` EVENT `event_fixedExpense_month` ON SCHEDULE EVERY 1 MONTH STARTS '2016-11-06 00:00:00' ON COMPLETION NOT PRESERVE ENABLE DO call fixed_expense_month()$$
